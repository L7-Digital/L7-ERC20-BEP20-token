...
contract TokensVesting is AccessControlEnumerable, ITokensVesting {
    ...
    bytes32 public constant GAME_INCENTIVES_ROLE = keccak256("GAME_INCENTIVES_ROLE");

    IERC20 public immutable token;

    VestingInfo[] private _beneficiaries;
    mapping(address => EnumerableSet.UintSet) private _beneficiaryAddressIndexes;
    ...
    function _vestedAmount(uint256 index_) private view returns (uint256) {
        ...
        uint256 _releaseMilestones = (_elapsedTime - _info.cliff) / _info.basis + 1;
        uint256 _totalReleaseMilestones = (_info.duration + _info.basis - 1) / _info.basis + 1;
            ...
            if (_info.beneficiary != address(0)) {
                ...
                if (_unreleaseAmount > 0) {
                    _info.releasedAmount =
                        _info.releasedAmount +
                        _unreleaseAmount;
                    token.transfer(_info.beneficiary, _unreleaseAmount);
    ...
    function _release(uint256 index_, address recipient_) private {
        ...
        if (_unreleaseAmount > 0) {
            ...
            token.transfer(recipient_, _unreleaseAmount);
    ...
    function _releaseTokensOfRole(
        ...
        token.transfer(reicipient_, amount_);
    ...
    /// FIXME: Add the address in whitelist before deployment
    function initAddBulkSeeding () private {
        // _addBeneficiary(seedingList[i][0], 0x00, genesisTimestamp_, totalAmount_, tgeAmount_, finalAmount_, basis_, cliff_, duration_, Participant.Seeding);
    }

    /// FIXME: Add the address in whitelist before deployment
    function initAddBulkPrivateSale () private {
        // _addBeneficiary(seedingList[i][0], 0x00, genesisTimestamp_, totalAmount_, tgeAmount_, finalAmount_, basis_, cliff_, duration_, Participant.Seeding);
    }
    ...
    function releaseMyTokens() external {
        ...
        for (
            ...
            if (_unreleaseAmount > 0) {
                _info.releasedAmount = _info.releasedAmount + _unreleaseAmount;
                token.transfer(_msgSender(), _unreleaseAmount);
